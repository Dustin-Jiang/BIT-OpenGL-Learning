...
void Circle::OnDraw()
{
	...
	else
	{
		...
		for (int i = 0; i < vertices.size(); i++)
		{
			...
			// 第一个圆周顶点
			float texU1 = (v1.x() / radius + 1.0f) * 0.5f;
			float texV1 = (v1.z() / radius + 1.0f) * 0.5f;
			glTexCoord2f(1.0 - texU1, 1.0 - texV1);
			glVertex3f(v1.x(), v1.y(), v1.z());

			// 第二个圆周顶点
			float texU2 = (v2.x() / radius + 1.0f) * 0.5f;
			float texV2 = (v2.z() / radius + 1.0f) * 0.5f;
			glTexCoord2f(1.0 - texU2, 1.0 - texV2);
